...
function setup() {
    ...
    bt1.addEventListener("click", ciao);
...
function ciao(){
    if(options[0].checked==true){
        ...
        var h=text.getAttribute("style");
        result_lb.setAttribute("style", h);

    }else if (options[1].checked==true) {

        result_lb.value=decrypt(text.value, key_el.value);;
        div_result.className += " is-upgraded is-dirty"
        var h=text.getAttribute("style");
        result_lb.setAttribute("style", h);
...
function lal(){
    ...
    for(var i=2; i<6; i++){
        var z=IC(text.value, i);
        if(ICmax<z){
            ICmax=z;
            len=i;
        }
        console.log(i + " " + IC(text.value, i));
    }
    console.log("ciao");
    string_chi(text.value, len);
    key_el.value=result;
    result_lb.value=decrypt(text.value, result);
